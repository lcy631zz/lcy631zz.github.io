...
DATA_FILE="data/yin.json"

echo -e "${GREEN}=== 添加二次元照片 ===${NC}"
echo ""
...
# 获取图片路径
echo -e "${YELLOW}请输入图片路径（例如: static/img/anime.jpg）:${NC}"
...
# 获取图片说明
echo -e "${YELLOW}请输入图片说明（例如: 角色名 - 作品名，直接回车跳过）:${NC}"
read -r CAPTION
...
print(f"\033[0;32m✓ 照片添加成功！\033[0m")
print(f"  图片: {img_path}")
if caption:
    print(f"  说明: {caption}")
print(f"\n  共 {len(data['items'])} 张照片")
PYEOF
